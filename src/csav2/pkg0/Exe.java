...
public class Exe {
    ...
    public static void Trial(){
        ...
        PolarPhraseIdentification pp = new PolarPhraseIdentification();
        stat.hmnegatives = pp.getNegativePolarityWordsHM();
        stat.hmpositives = pp.getPositivePolarityWordsHM();
        
        PolarityClassification pc = new PolarityClassification();
        stat.hmblpos = pc.getBLPositiveWordsHM();
        stat.hmblneg = pc.getBLNegativeWordsHM();
        stat.hmvspos = pc.getVSPositiveWordsHM();
        stat.hmvsneg = pc.getVSNegativeWordsHM();
        ...
        for(int i=0; i<stat.globalcounter; i++){
            ...
            stat.fourgrams[i] = f.getFourgramsWF(stat.words[i]);
            ...
            stat.fourgmatchp[i] = f.compareHashmaps(f.getFourgramsWF(stat.words[i]),stat.hmpositives);
            //System.out.println("Trip:"+stat.trimatchp[i]);
            stat.positivewords[i] = stat.unimatchp[i] + stat.bimatchp[i] + stat.trimatchp[i] + stat.fourgmatchp[i];
            ...
            stat.fourgmatchn[i] = f.compareHashmaps(f.getFourgramsWF(stat.words[i]), stat.hmnegatives);
            stat.negativewords[i] = stat.unimatchn[i] + stat.bimatchn[i] + stat.trimatchn[i] + stat.fourgmatchn[i];
            //System.out.println("Negative:"+stat.negativewords[i]);
            
            // Obtaining match with BL's positive polar phrases
            stat.blpos[i] = u.compareHashmaps2(u.getUnigramsWF(stat.words[i]),stat.hmblpos);
            
            // Obtaining match with BL's negative polar phrases
            stat.blneg[i] = u.compareHashmaps2(u.getUnigramsWF(stat.words[i]),stat.hmblneg);
            
            // Obtaining match with VS's positive polar phrases
            stat.vspos[i] = u.compareHashmaps2(u.getUnigramsWF(stat.words[i]),stat.hmvspos);
            
            // Obtaining match with VS's negative polar phrases
            stat.vsneg[i] = u.compareHashmaps2(u.getUnigramsWF(stat.words[i]),stat.hmvsneg);  
    ...
    public static void Test(){
        ...
        PolarPhraseIdentification pp = new PolarPhraseIdentification();
        stat_test.hmnegatives = pp.getNegativePolarityWordsHM();
        stat_test.hmpositives = pp.getPositivePolarityWordsHM();
        
        PolarityClassification pc = new PolarityClassification();
        stat_test.hmblpos = pc.getBLPositiveWordsHM();
        stat_test.hmblneg = pc.getBLNegativeWordsHM();
        stat_test.hmvspos = pc.getVSPositiveWordsHM();
        stat_test.hmvsneg = pc.getVSNegativeWordsHM();
        ...
        for(int i=0; i<stat_test.globalcounter; i++){
            ...
            stat_test.fourgrams[i] = f.getFourgramsWF(stat_test.words[i]);
            //if (stat.trigrams[i]==null) System.out.println("Error trigrams");
            ...
            stat_test.fourgmatchp[i] = f.compareHashmaps(f.getFourgramsWF(stat_test.words[i]),stat_test.hmpositives);
            //System.out.println("Trip:"+stat.trimatchp[i]);
            stat_test.positivewords[i] = stat_test.unimatchp[i] + stat_test.bimatchp[i] + stat_test.trimatchp[i] + stat_test.fourgmatchp[i];
            //System.out.println("Positive:"+stat.positivewords[i]);
            ...
            stat_test.fourgmatchn[i] = f.compareHashmaps(f.getFourgramsWF(stat_test.words[i]),stat_test.hmnegatives);
            //System.out.println("Trin:"+stat.trimatchn[i]);
            stat_test.negativewords[i] = stat_test.unimatchn[i] + stat_test.bimatchn[i] + stat_test.trimatchn[i] + stat_test.fourgmatchn[i];
            //System.out.println("Negative:"+stat.negativewords[i]);
            
            // Obtaining match with BL's positive polar phrases
            stat_test.blpos[i] = u.compareHashmaps2(u.getUnigramsWF(stat_test.words[i]),stat_test.hmblpos);
            
            // Obtaining match with BL's negative polar phrases
            stat_test.blneg[i] = u.compareHashmaps2(u.getUnigramsWF(stat_test.words[i]),stat_test.hmblneg);
            
            // Obtaining match with VS's positive polar phrases
            stat_test.vspos[i] = u.compareHashmaps2(u.getUnigramsWF(stat_test.words[i]),stat_test.hmvspos);
            
            // Obtaining match with VS's negative polar phrases
            stat_test.vsneg[i] = u.compareHashmaps2(u.getUnigramsWF(stat_test.words[i]),stat_test.hmvsneg);  
